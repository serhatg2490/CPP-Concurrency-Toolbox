...
Five additional hypotheses were tested, and all five were directly refuted by measurement:
...
A total of **10 independent interventions/hypotheses** were tested by direct measurement: wrong core pinning (fixed, real benefit), C-state, IRQ/timer tick (`isolcpus`/`nohz_full`, 99% reduction), our own code's IPI source (tested with mallopt+thread pool, 87% reduction measured, then reverted), HT-sibling contention (demonstrably cleaned up), RT throttling, `SCHED_FIFO` priority, queue-full/backpressure, thermal/turbo throttling, and P-state/frequency-floor ramp jitter. **None** of them moved the SPSC 1C / Vyukov 1C ~180–250µs P99.9 / ~570–630µs Max band; this band has stayed almost exactly the same size since the very start of the investigation (in both steady_clock and TSC, across both queue implementations).
...
3. Disable deep C-states (C6/C10) on those same cores (§9).
